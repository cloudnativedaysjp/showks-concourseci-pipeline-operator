...
type ConcourseCIClientInterface interface {
	...
	ExposePipeline(target string, pipeline string) error
}
...
func (c *ConcourseCIClient) ExposePipeline(target string, pipeline string) error {
	err := c.Login()
	if err != nil {
		return err
	}
	...
	if err != nil {
		return errors.Wrapf(err, "Failed to ExposePipeline: %s", out)
	}
